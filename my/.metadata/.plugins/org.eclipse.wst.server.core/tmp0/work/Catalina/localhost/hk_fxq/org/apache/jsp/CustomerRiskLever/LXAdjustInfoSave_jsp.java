...
public final class LXAdjustInfoSave_jsp extends org.apache.jasper.runtime.HttpJspBase
...
catch(Exception exception)
{
    ...
    String finalScore2 = new String(request.getParameter("finalScore2"));
  //  String finalLevel2 = new String(request.getParameter("finalLevel2"));
    String finalLevel2 = new String(request.getParameter("finalLevel2").getBytes("ISO-8859-1"),"GBK");
	String record = new String(request.getParameter("record2"));
    String no = new String(request.getParameter("no"));
    //String finalRemark = new String(request.getParameter("FinalRemark"));
    String finalRemark = new String(request.getParameter("FinalRemark").getBytes("ISO-8859-1"),"GBK");
	
    VData tVData = new VData();
    LXAdjustInfoUI lxAdjustInfoUI = new LXAdjustInfoUI();  
	LXCustomerRiskAdjustSchema lxCustomerRiskAdjustScheme = new LXCustomerRiskAdjustSchema();
	lxCustomerRiskAdjustScheme.setADJUSTSCORE(finalScore2);
	lxCustomerRiskAdjustScheme.setADJUSTLEVEL(finalLevel2);
	lxCustomerRiskAdjustScheme.setOPORETOR(oper);
	lxCustomerRiskAdjustScheme.setADJUSTREMARK(finalRemark);
	lxCustomerRiskAdjustScheme.setNOB(no);
	LXCustomerRiskResultAllSchema lxCustomerRiskResultAllScheme = new LXCustomerRiskResultAllSchema();
	lxCustomerRiskResultAllScheme.setFINALSCORE(finalScore2);
	lxCustomerRiskResultAllScheme.setFINALLEVEL(finalLevel2);
	lxCustomerRiskResultAllScheme.setFINALREMARK(finalRemark);
    ...
    {
        System.out.println("========= In LXAdjustInfoSave.jsp ========== =Operate=" + Operate);
        try
        {
        	tVData.add(tGI);
            tVData.add(lxCustomerRiskAdjustScheme);
            tVData.add(lxCustomerRiskResultAllScheme);
            lxAdjustInfoUI.submitData(tVData, Operate);
        }
        catch(Exception ex)
        {
        	ex.printStackTrace();
