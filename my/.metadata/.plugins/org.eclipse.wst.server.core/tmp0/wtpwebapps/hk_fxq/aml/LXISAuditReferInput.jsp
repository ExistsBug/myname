...
<head>
    ...
    <!-- 私有引用脚本 -->
    <script language="JavaScript" src="LXISAuditRefer.js"></script>
    <%@ include file="LXISAuditReferInit.jsp" %>
</head>
<body topmargin="0" onload="initForm()" ondragstart="return false">
    <form name="fm" method="post" target="fraSubmit">
        ...
        <div id="divWaitForBeFileGrid" style="display:''">
            ...
            <INPUT class=cssButton VALUE="同意上报" name="affirm" TYPE=button onclick="SubmitWaitForBeFileGrid(this.name);" />
            
            <INPUT class=cssButton VALUE="同意不上报" name="keep" TYPE=button onclick="SubmitWaitForBeFileGrid(this.name);" />
            <INPUT class=cssButton VALUE="审批不通过" name="back" TYPE=button onclick="SubmitWaitForBeFileGrid(this.name);" />
            <!--INPUT class=cssButton VALUE="放弃打回" name="sdfsdfdsfds" TYPE=button onclick="checkdataall(this.name);" /-->
        </div>
        <!-- 获取数据的隐藏域 -->
        ...
        <table>
         <tr>
                ...
                <td class="titleImg">同意不上报或审批不通过原因</td>
         </tr>
        </table>
