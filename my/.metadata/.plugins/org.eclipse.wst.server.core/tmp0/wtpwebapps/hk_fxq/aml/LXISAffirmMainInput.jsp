...
<!-- 私有引用脚本 -->
<script language="JavaScript" src="LXISAffirmMain.js"></script>
<%@ include file="LXISAffirmMainInit.jsp"%>
</head>
<body topmargin="0" onload="initForm()" ondragstart="return false">
	<form name="fm" method="post" target="fraSubmit">
		...
		<div id="divWaitForBeFileSearch" style="display: ''">
			...
			<table class="common">
				...
				<tr class="common">
					...
					<TD class=input><Input class=codeno name="StatiCode" readonly
						onDblClick="return showCodeList('station46',[this,StatiCodeName],[0,1]);"
						onKeyUp="return showCodeListKey('station46',[this,StatiCodeName],[0,1]);"><input
						class=codename name="StatiCodeName" readonly=true></TD>
					<td class="title">规则原因</td>
					<TD class=input><Input class=codeno name="RuleReason" readonly
						onDblClick="return showCodeList('amlshadinessstamp',[this,RuleReasonName],[0,1]);"
						onKeyUp="return showCodeListKey('amlshadinessstamp',[this,RuleReasonName],[0,1]);"><input
						class=codename name="RuleReasonName" readonly=true></TD>
					<td class="title">交易类型</td>
					<TD class="input"><Input name="TSTP" readonly id="TSTP"
						class="codeno" verify="交易类型|notnull&Code:amltranstype"
						ondblclick="showCodeList('amltranstype',[this,TSTPName],[0,1]);"
						onkeyup="showCodeListKey('amltranstype',[this,TSTPName],[0,1]);"><Input
						class=codename name=TSTPName readOnly elementtype=nacessary>
				</tr>
			</table>
			<!-- 提交数据操作按钮 -->
			<input type="button" class="cssButton" value=" 查 询 "
				onclick="queryWaitForBeFileGrid(fm.UserCode.value)">
		</div>
		<!-- 交易信息折叠展开 -->
		<table>
			<tr>
		...
		<div id="divWaitForBeFileGrid" style="display: ''">
			...
			<br>
			<!-- 提交数据操作按钮 -->
			<INPUT class=cssButton VALUE="查看交易信息" name="update" TYPE=button
			...
			<INPUT class=cssButton VALUE="提交上级审核" name="affirm" TYPE=button
				onclick="SubmitWaitForBeFileGrid(this.name);" /> <INPUT
				class=cssButton VALUE="放弃上报" name="keep" TYPE=button
				onclick="SubmitWaitForBeFileGrid(this.name);" />
		</div>
		<!-- 获取数据的隐藏域 -->
		<input type="hidden" name="Operation"> <input type="hidden"
			name="UserCode" value="<%=OperatorCodee%>">
	</form>
	<!-- 通用下拉信息列表 -->
