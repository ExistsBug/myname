...
<head>
    ...
    <!-- 私有引用脚本 -->
    <script language="JavaScript" src="largeTradeRsentFileMake.js"></script>
    <%@ include file="largeTradeReSentFileMakeInit.jsp" %>
</head>
<body topmargin="0" onload="initForm();initElementtype();" ondragstart="return false">
    <form name="fm" method="post" target="fraSubmit"  action="largeTradeReSentToXmlSave.jsp?type=applyFileNo">
        <!-- 报文查询折叠展开 -->
        <table>
            <tr>
     ...
     <TD class=title>报文类型</TD>
                    <TD class= input>
                        <Input class="codeno" name=dataType  readonly value="<%=tDataType%>" readonly><input class=codename name=dataTypeName value='' readonly>
                </TD>
                              <td class="title">&nbsp;</td>
                <td class="input">&nbsp;</td>
                <td class="title">&nbsp;</td>
            ...
            <input type="button" class="cssButton" value=" 申 请 " onclick="applyANewFileMsg()">
            <br>
        </div>
        <!-- 报文信息折叠展开 -->
        <table>
