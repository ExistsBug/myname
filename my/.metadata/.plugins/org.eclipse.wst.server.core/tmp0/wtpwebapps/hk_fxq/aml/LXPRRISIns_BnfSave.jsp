...
System.out.println(" =========== In LXPRRISIns_BnfSave.jsp Start ============ ");
LXPRRISIns_BnfUI tLXPRRISIns_BnfUI = new LXPRRISIns_BnfUI();
if(!tLXPRRISIns_BnfUI.submitData(tVData,""))
{
	FlagStr = "Fail";
	Content = "导出失败，原因是："+tLXPRRISIns_BnfUI.mErrors.getFirstError();
}
else
{
	mResult = tLXPRRISIns_BnfUI.getResult();	
	...
	{
		"序号"," 保险合同号","被保险人名称/姓名","被保险人身份证件/证明文件类型","被保险人身份证件/证明文件号码",
		"保费","受益人名称/姓名","受益人身份证件/证明文件类型","受益人身份证件/证明文件号码","备注","交易编号"
	};
	tLB.data=mResult;          
	tLB.row1=0;                
	tLB.col1=0;                
	...
	{	                     
  	  ...
  	  bos.flush();           
	    bos.close();           
	    System.out.println("===== IN LXPRRISIns_BnfSave.jsp End =====");
	    FlagStr = "Succ";      
	    Content = "导出成功！";
	}                          
	catch(Exception e)         
