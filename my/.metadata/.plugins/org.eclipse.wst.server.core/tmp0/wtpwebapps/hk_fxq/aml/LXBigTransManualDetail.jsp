...
<jsp:include page="BigTransDetailPage.jsp"/>
		...
		<INPUT class=cssButton VALUE=" 添加大额交易 " name="subbutton" TYPE=button onclick="return submitForm();" />
...
<script>
	...
	if (tOper == "insert"){
		...
		//增加交易时间初始化查询，不用手录
	var tSQL = " select substr(replace(htdt,'-',''),1,8)||replace(dealtime,':','') from lxihtrademain "
		         + " where dealno='"+fm.DealNo.value+"' ";
		var tResult  = easyQueryVer3(tSQL, 1, 0, 1);
		var arr_R = decodeEasyQueryResult(tResult);
		fm.TSTM.value=arr_R[0][0];		
	} 
	else
	{ 
		if (tOper == "update"){
	...
	var arr = "";
	if(fm.all("TDRC").value==null||fm.all("TDRC").value=='')
	{
	  var tMngCom = <%= tGI.ComCode %>;
	  if(tMngCom.length>=4)
	  {
	   // tMngCom = tMngCom.substring(0,4);
	  }
	  var tSQL = " SELECT CODEname FROM LDCODE  WHERE CODE= '"+tMngCom+"' "
	           + " and codetype = 'regcode' ";
	  arr=easyExecSql(tSQL); 
	          
	  if(arr[0][0]!=null)
	  {
	    fm.all("TDRC").value="CHN"+arr[0][0];
	  }
	}
	if(fm.all("TRCD").value==null||fm.all("TRCD").value=="")
	{
	  if(fm.all("TDRC").value!=null)
	  {
	    fm.all("TRCD").value=fm.all("TDRC").value;
	  }
	}
	if(fm.all("FIRC").value==null||fm.all("FIRC").value=="")
	{
	  fm.all("FIRC").value=arr[0][0];
		if(fm.all("FIRC").value == "000000" )
		{
			fm.all('FIRC').value = "@N"
		}
	}
		if(fm.all("TSTP").value==null||fm.all("TSTP").value=="")
	{
	  fm.all("TSTP").value="000030";
	}
