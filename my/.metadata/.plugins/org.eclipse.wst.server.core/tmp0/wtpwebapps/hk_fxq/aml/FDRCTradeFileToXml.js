...
function checkCanDownloadFileMsg()
{
    ...
    if (arrResult != null)
    {
        try
        {
            ...
            document.getElementsByName("AddDealNo")[0].disabled = true;
            document.getElementsByName("DeleteDealNo")[0].disabled = true;
            var strQueryResult = easyQueryVer3(QuerySQL, 1, 0, 1);
            var arrDataSet = decodeEasyQueryResult(strQueryResult);
            packagePath="../aml/send/"+arrDataSet[0][1];
        }
        catch (ex) {}
    }
    else
...
function queryWaitForBeFileGrid()
{
	...
	strSQL+="and not exists (select 'X' from LXReportDetail where LXIHTradeMain.dealno=LXReportDetail.dealno "
	       +" and exists (select 'X' from LXReportDataFile where 1 = 1 and FileNo = LXReportDetail.FileNo "
	       +" and ReportType = '01' and DealState in ('00', '01', '02', '06'))) and datastate in "+tradeDataState;
...
function addToInFileMsg()
{
    ...
    else
    {
        ...
        document.forms(0).action = "FDRCTradeToXmlSave.jsp";
...
function makeAllInFileMsg()
{
    ...
    try
    {
        fm.type.value = "MakeAllInFileMsgFDRC";
    }
    catch (ex) {}
    document.forms(0).action = "FDRCTradeToXmlSave.jsp";
    document.forms(0).submit();
...
function GoToViewAndUpdate(str)
{
    ...
    if (nCheckCount == null || nCheckCount <= 0)
    {
     ...
     if (isOneMore) 
     {
         ...
         var strUrl = "../aml/LXBigTransManualToMain.jsp";
