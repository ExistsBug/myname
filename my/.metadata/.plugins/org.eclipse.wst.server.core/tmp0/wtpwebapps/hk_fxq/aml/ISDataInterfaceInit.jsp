...
function initInpBox()
{
  try
  {
    ...
    fm.all('STCR').value = '';
    fm.all('MngCom').value = "<%=tGI.ComCode%>";//登陆机构
    initTasksGrid();
  ...
  {
    alert("ISDataInterfaceInit.jsp-->InitInpBox函数中发生异常:初始化界面错误!");
  }
}
...
function initInpBox1()
{
  try
  {
    ...
    fm.all('EndDate').value = '';
    fm.all('STCR').value = '';
    fm.all('CalMngCom').value = '';
    fm.all('CalMngComName').value = '';
  }
  catch(ex)
  {
    alert("ISDataInterfaceInit.jsp-->InitInpBox函数中发生异常:初始化界面错误!");
...
function initForm()
{
  ...
  {
    alert("ISDataInterfaceInit.jsp-->InitForm函数中发生异常:初始化界面错误!");
  }
}
