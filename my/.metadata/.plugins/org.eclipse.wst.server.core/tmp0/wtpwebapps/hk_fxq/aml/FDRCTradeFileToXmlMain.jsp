...
<frameset name="fraMain" framespacing="0" frameborder="no" rows="0,0,0,0,*" cols="*" border="1">
    ...
    <frameset name="fraSet" framespacing="0" frameborder="no" rows="*" cols="0%,*,0%" border="1">
        ...
        <!-- 实现交互的区域 -->
        <frame id="fraInterface" name="fraInterface" scrolling="auto" src="FDRCTradeFileToXmlInput.jsp?FileNo=<%=request.getParameter("FileNo")%>&dataType=<%=request.getParameter("dataType")%>&reportType=<%=request.getParameter("reportType")%>">
        <!-- 下一步操作区域 -->
